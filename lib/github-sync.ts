...
export async function createSamplePipelineData(input: SyncRepoInput): Promise<SyncResult> {
  ...
  const pipeline = await prisma.pipeline.upsert({
    where: { id: `sample-${input.projectId}` },
    update: {
      status: "FAILED",
      lastRunAt: endedAt,
      durationSec: 360
    ...
    create: {
      id: `sample-${input.projectId}`,
      orgId: input.orgId,
      projectId: input.projectId,
      name: `${input.repo}-workflow`,
      service: input.repo,
...
export async function syncGithubWorkflowRunsForRepo(input: SyncRepoInput): Promise<SyncResult> {
  ...
  for (const run of workflowRuns) {
    ...
    const pipelineId = `ghwf-${input.projectId}-${run.workflow_id ?? run.name.toLowerCase().replace(/\W+/g, "-")}`;
